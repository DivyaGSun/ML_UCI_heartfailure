...
apply(trainData[, 1:12], 2, FUN=function(x){c('min'=min(x), 'max'=max(x))})

# Visualize Features

png("boxplot_features.png")
...
model_RF
png("rf_features.png")
e <- plot(varImp(model_RF, scale=FALSE), main ='Random Forest')
dev.off()
# train model log
...
#models <- list(model_LDA, model_cart, model_KNN, model_svm, model_RF, model_log)
#names <- list("LDA", "cart", "KNN", "svm", "RF")
